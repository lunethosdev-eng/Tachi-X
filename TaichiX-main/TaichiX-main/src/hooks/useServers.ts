...
export function useServers() {
  ...
  const startServer = useCallback(
    async (id: string) => {
      ...
      try {
        // El runtime Java Android se prepara solo la primera vez, sin Termux.
        let avail = { javaRuntime: false, playit: false };
        if (server.type === 'java') {
          await appendLogs(id, ['Comprobando runtime Java Android ARM64...']);
          const runtime = await ServerNative.ensureJavaRuntime();
          if (!runtime.success) {
            await appendLogs(id, [runtime.message || 'No se pudo preparar Java']);
            await setStatus(id, 'error');
            return;
          }
          await appendLogs(id, [runtime.message]);
        }
        ...
        if (server.type === 'java' && !avail?.javaRuntime) {
          await appendLogs(id, ['❌ El runtime Java Android no pudo ejecutarse en este dispositivo/ABI.']);
          await setStatus(id, 'error');
          return;
        }
