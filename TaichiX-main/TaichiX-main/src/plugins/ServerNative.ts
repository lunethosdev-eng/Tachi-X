...
export interface ServerNativePlugin {
  ...
  ensureJavaRuntime(): Promise<{ success: boolean; path?: string; message: string }>;
...
export default ServerNativePlugin || ({
  ...
  ensureJavaRuntime: async () => ({ success: false, message: 'Runtime Java disponible solo en Android nativo' }),
