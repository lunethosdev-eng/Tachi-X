...
public class ServerNativePlugin extends Plugin {
    ...
    @PluginMethod
    public void ensureJavaRuntime(PluginCall call) {
        executor.execute(() -> {
            try {
                File runtimeDir = new File(getContext().getFilesDir(), "runtime/java");
                File javaBin = new File(runtimeDir, "bin/java");
                if (javaBin.exists() && javaBin.canExecute()) {
                    JSObject ret = new JSObject(); ret.put("success", true); ret.put("path", javaBin.getAbsolutePath()); ret.put("message", "Runtime Java Android ARM64 ya instalado"); call.resolve(ret); return;
                }
                if (!runtimeDir.exists() && !runtimeDir.mkdirs()) { call.reject("No se pudo crear runtime/java"); return; }
                File archive = new File(getContext().getFilesDir(), "runtime/jre17-arm64.tar.xz");
                JSObject progress = new JSObject(); progress.put("message", "Descargando runtime Java Android ARM64..."); progress.put("progress", 0); notifyListeners("runtimeProgress", progress);
                if (!archive.exists() || archive.length() < 1000000) {
                    String url = "https://github.com/itsaky/openjdk-17-android/releases/download/01-01-2022/jre17-arm64.tar.xz";
                    if (!downloadFile(url, archive)) { call.reject("No se pudo descargar el runtime Java Android ARM64"); return; }
                }
                extractRuntime(archive, runtimeDir);
                findAndFlattenJava(runtimeDir);
                javaBin = new File(runtimeDir, "bin/java");
                if (!javaBin.exists()) { call.reject("El runtime descargado no contiene bin/java"); return; }
                javaBin.setExecutable(true); chmodTree(runtimeDir);
                archive.delete();
                JSObject ret = new JSObject(); ret.put("success", true); ret.put("path", javaBin.getAbsolutePath()); ret.put("message", "Runtime Java Android ARM64 listo"); call.resolve(ret);
            } catch (Exception e) { call.reject("Error preparando Java: " + e.getMessage()); }
        });
    }

    private void extractRuntime(File archive, File destination) throws Exception {
        try (InputStream fin = new FileInputStream(archive); XZCompressorInputStream xz = new XZCompressorInputStream(fin); TarArchiveInputStream tar = new TarArchiveInputStream(xz)) {
            ...
            while ((entry = tar.getNextTarEntry()) != null) {
                ...
                if (!out.getCanonicalPath().startsWith(destination.getCanonicalPath() + File.separator)) continue;
                if (entry.isDirectory()) { out.mkdirs(); continue; }
                File parent = out.getParentFile(); if (parent != null) parent.mkdirs();
                try (FileOutputStream fos = new FileOutputStream(out)) { int n; while ((n = tar.read(buf)) > 0) fos.write(buf, 0, n); }
            }
        }
    }

    private void findAndFlattenJava(File runtimeDir) throws Exception {
        File direct = new File(runtimeDir, "bin/java"); if (direct.exists()) return;
        File[] children = runtimeDir.listFiles(); if (children == null) return;
        for (File child : children) { File candidate = new File(child, "bin/java"); if (candidate.exists()) { File[] files = child.listFiles(); if (files != null) for (File f : files) f.renameTo(new File(runtimeDir, f.getName())); child.delete(); return; } }
    }

    private void chmodTree(File root) { File[] files = root.listFiles(); if (files == null) return; for (File f : files) { if (f.isDirectory()) chmodTree(f); else if (f.getName().equals("java") || f.getName().endsWith(".so")) f.setExecutable(true); } }
    ...
    public void startServer(PluginCall call) {
        ...
        executor.execute(() -> {
            try {
                ...
                Intent serviceIntent = new Intent(getContext(), TaichiXServerService.class);
                serviceIntent.putExtra("title", "Servidor " + serverId + " activo");
    ...
    public void stopServer(PluginCall call) {
        ...
        executor.execute(() -> {
            try {
                stopProcess(processes, serverId);
                if (processes.isEmpty()) getContext().stopService(new Intent(getContext(), TaichiXServerService.class));
                JSObject ret = new JSObject();
                ret.put("success", true);
